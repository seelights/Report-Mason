...
ContentExtractor::ContentExtractor(QObject *parent)
    : QObject(parent)
    , m_outputDirectory(QStandardPaths::writableLocation(QStandardPaths::TempLocation) + "/ReportMason")
    , m_idCounter(0)
{
    // 确保输出目录存在
...
QString ContentExtractor::getLastError() const
{
    return m_lastError;
}
...
QString ContentExtractor::getOutputDirectory() const
{
...
QString ContentExtractor::generateUniqueId(const QString &prefix)
{
    ...
    return QString("%1_%2_%3").arg(prefix, timestamp, counter);
}
...
bool ContentExtractor::saveContentToFile(const QByteArray &content, const QString &fileName)
{
    ...
    if (!file.open(QIODevice::WriteOnly)) {
        setLastError(QString("无法创建文件: %1").arg(fullPath));
        return false;
    }
    ...
    if (bytesWritten != content.size()) {
        setLastError(QString("文件写入不完整: %1").arg(fullPath));
        return false;
    }
