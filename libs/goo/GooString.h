...
public:
    ...
    // Return true if strings starts with prefix (C++17 compatibility)
    bool starts_with(const std::string &prefix) const {
        return size() >= prefix.size() && compare(0, prefix.size(), prefix) == 0;
    }
    bool starts_with(const char *prefix) const {
        size_t len = strlen(prefix);
        return size() >= len && compare(0, len, prefix) == 0;
    }

    // Return true if string ends with suffix (C++17 compatibility)
    ...
    bool ends_with(const char *suffix) const {
        ...
        return size() >= len && compare(size() - len, len, suffix) == 0;
    }
};
