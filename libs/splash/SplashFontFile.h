...
public:
    ...
    const unsigned char* getBuf() const { return buf.empty() ? nullptr : buf.data(); }
    int getBufLen() const { return buf.size(); }
    bool isFileSource() const { return isFileFlag; }
    bool isFile() const { return isFileFlag; } // Alias for compatibility
